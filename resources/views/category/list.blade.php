<table class="table table-hover table-bordered">
    ...
    <tbody>
    @forelse(\App\Category::with('user')->get()->all() as $category)
        <tr>
            <td>{{$category->id}}</td>
            ...
            <td>
                ...
                <form action="{{route('category.destroy',$category->id)}}" class="d-inline-block" method="post">
                    @csrf
                    @method('delete')
                    <button class="btn btn-outline-danger" onclick="return confirm(`R U sure to delete '{{$category->title}}' category?`)">Delete</button>
                </form>
            </td>
            <td>
                <span class="small">
